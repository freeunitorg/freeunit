...
# block sits 1024 below the base, so a lower base would need a privileged
# port).  The upper bound is 32767 rather than the 65535 the highest output
# would allow, to keep the whole band under the ephemeral range.
MIN_BASE = 7876
MAX_BASE = 32767
...
def set_base(base):
    ...
    for _, literal, value in entries:
        if value != literal and value in literals:
            raise ValueError(
    ...
    _BASE = base
    _MAPPED_OUTPUTS = frozenset(
        str(value) for _, literal, value in entries if value != literal
    )

    # Mirrored for tests that assert on the port the server was configured
    # with.  Never read as the source of truth: see the module docstring.
    option.base_port = base
...
def remap(text):
    ...
    only when it is not already the base's own value.  That second guard
...
def _mapped_entries(base):
    """(kind, literal, mapped value) for every literal the map knows."""
    return [
        ('listener', literal, base + offset)
        for literal, offset in _BAND.items()
    ] + [
        ('helper', literal, base + _BLOCK_OFFSET + (literal - _BLOCK_FIRST))
