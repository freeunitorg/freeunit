...
|------|-------|-----------|--------------|--------|
| 7977 | — | — (plain-Python upstream in the test file) | — | `test_proxy_interim.py` (upstream 1xx interim responses -- 100, 103, several in a row, one larger than half the header buffer, and a 100 drawn by a forwarded `Expect: 100-continue` -- are consumed and the final response relayed) |
