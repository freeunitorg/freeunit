 ...
 *     send;
 *   - the wait stays bounded anyway -- the next expiry answers, so a worker
 *     that claims a request and then hangs cannot hold it forever;
...
nxt_router_app_timeout_test(nxt_thread_t *thr)
{
    ...
    if (claimed.r->timer.handler != nxt_router_test_app_timeout
        || !claimed.r->timer.enabled)
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log,
                      "router app timeout test: the deadline was not re-armed "
                      "for a claimed request");
        goto done;
    }

    /*
     * Leg 2: the worker that claimed the request never acknowledges it.  The
     * next expiry must answer, or a claim would hold a request forever.
     */
    ...
    if (claimed.r->error == 0 || claimed.r->req_rpc_data != NULL) {
        nxt_log_error(NXT_LOG_NOTICE, thr->log,
                      "router app timeout test: a claimed request that never "
                      "answered was not bounded by the next deadline");
        goto done;
    }

    if (nxt_router_app_timeout_test_completions != 2
        || claimed.header->is_port_mmap_sent != 1)
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log,
                      "router app timeout test: the message of a claimed "
                      "request was not left to the worker (%ui buffers, "
                      "sent %d)",
                      nxt_router_app_timeout_test_completions,
                      (int) claimed.header->is_port_mmap_sent);
        goto done;
    }
