...
nxt_port_fail_test_enqueue(nxt_task_t *task, nxt_port_t *port, nxt_mp_t *mp,
    nxt_err_t err, nxt_uint_t fails, nxt_bool_t no_memory)
{
    nxt_buf_t  *buf;
    nxt_int_t  ret;
...
nxt_port_fail_test_wakeup_errno(nxt_thread_t *thr)
{
    ...
    for (i = 1; i <= 2; i++) {
        ...
        if (nxt_port_fail_test_enqueue(task, port, mp, NXT_ENOMEM, 1, 0)
            != NXT_OK)
        {
            goto done;
        }
    ...
    if (nxt_port_fail_test_enqueue(task, port, mp, NXT_ENOMEM, 2, 0)
        != NXT_OK)
    {
        goto done;
    }
    ...
    if (nxt_port_fail_test_enqueue(task, port, mp, NXT_ENOMEM, 1, 1)
        != NXT_OK)
    {
        goto done;
    }
    ...
    if (nxt_port_fail_test_enqueue(task, port, mp, NXT_EPIPE, 1, 0) != NXT_OK) {
        goto done;
    }
    ...
    if (!nxt_queue_is_empty(&port->messages) || port->announce != 0) {
        nxt_log_error(NXT_LOG_NOTICE, thr->log,
                      ...
                      "for a peer that is gone (announce %d)",
                      (int) port->announce);
        goto done;
    }
