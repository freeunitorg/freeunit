    ...
    nxt_http_static_ctx_t *ctx)
{
    ...
    *f = file;
    ...
    if (nxt_fast_path(nxt_is_file(&fi))) {
         ...
         *
         * Because a coded range cannot be validated on resume.  gzip output
         * is not stable across zlib versions, levels or restarts, and the
         * entity-tag is deliberately weakened for a coded representation, so
         * "If-Range" can never match one.  A client resuming a coded range
         * would splice bytes from two different compressions with nothing
         * able to detect it -- a corrupt file rather than a stale one.
         *
